...
def system_user_update(user,systemrole,status,initials,firstname,lastname):
  ...
  #print(dict(row))
  msg = "Updated system user: " + str(user) + ", " + str(systemrole) + ", " + str(status) + ", " + str(initials)
  logmsg("INFO", msg)
  return msg
...
def execute_sql_command(sql_cmd):
  ...
  if sql_cmd[0:6] == "SELECT" or sql_cmd[0:4] == "CALL":
    conn.ping(reconnect=True)
    ...
    msg = "FAIL: SQL command failed (only allowed SQL command starting with SELECT or CALL): " + sql_cmd
    logmsg("ERROR",msg)
